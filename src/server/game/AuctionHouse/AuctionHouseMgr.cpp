...
void AuctionHouseObject::SendAuctionSold(AuctionPosting const* auction, Player* owner, CharacterDatabaseTransaction trans) const
{
    ...
    if ((owner || sCharacterCache->HasCharacterCacheEntry(auction->Owner)) && !sAuctionBotConfig->IsBotChar(auction->Owner))
    {
        ...
        // 积分拍卖行，货币类型：0金币 1泡点 2魔币 3积分 4战场分数 5物品
        uint32 accountid = sCharacterCache->GetCharacterAccountIdByGuid(auction->Owner);
        std::string danwei = aaCenter.AA_GetAuctionUnit();
        uint64 token = auction->BuyoutOrUnitPrice;
        int32 sx = 0;
        if (aaCenter.aa_world_confs[61].value1 > 0)
        {
