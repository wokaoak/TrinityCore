...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId, " \
    ...
    PrepareStatement(_AA_UPD_Character_Paihang_Hongbao, "update _玩家排行数据x set 红包发放总金额 = ? where guid = ?", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Character_Instance, "replace into _玩家装备数据 (guid, itemEntry, owner_guid, name, 原始组, 原始属性, 鉴定组, 鉴定id, 鉴定等级, 鉴定属性, 强化组, 强化id, 强化等级, 强化属性, 强化奖励属性, 强化奖励属性百分比, 强化奖励技能, 成长组, 成长id, 成长等级, 成长经验, 成长属性, 成长奖励属性, 成长奖励属性百分比, 成长奖励技能, 附魔技能数量, 附魔技能, 附魔技能锁定状态, 附魔属性数量, 附魔属性, 附魔属性锁定状态, 重铸次数, 重铸属性需要, 重铸技能需要, 符文数量, 符文属性, 符文组合属性, 萃取位置, 宝石id, 随机套装组, 租赁时间, update_time) VALUES (?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Character_ZhanPet, "replace into _玩家战宠数据 (id, owner_guid, 宠物_id, 等级, 经验, 宠物_等级_组, 成长, 契合, 继承, 生命, 伤害, 法术, 治疗, 护甲, 宠物技能, 主人技能, 已出战, 已合体, update_time) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_DEL_Character_ZhanPet, "DELETE FROM _玩家战宠数据 WHERE id = ?", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Characters, "replace into _玩家角色数据 (guid, 在线奖励领取状态, 升级奖励领取状态, 天赋点, 累计抽奖次数, 累计抽奖领取, 物品购买次数, 物品永久购买, 招募者姓名, 自定义角色变量, 名字前缀, 名字后缀, 自定义阵营, 一命等级, 一命模式, 活跃值, 活跃进度, 活跃进度领取状态, update_time) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Systems, "replace into _系统数据 (id, 自定义系统变量, update_time) VALUES(?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Characters_Dianfeng, "replace into _玩家巅峰属性 (guid, 等级, 经验, 总点数, 消耗点数, 天赋点, 属性值, update_time) VALUES(?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Characters_Douqi, "replace into _玩家斗气属性 (guid, 等级, 经验, 总点数, 消耗点数, 天赋点, 属性值, update_time) VALUES(?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Characters_Junxian, "replace into _玩家军衔属性 (guid, 等级, 经验, 总点数, 消耗点数, 天赋点, 属性值, update_time) VALUES(?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    
    PrepareStatement(_AA_REP_Player_Map_Value, "insert into _数据玩家map (guid, map, 布尔变量, 数值变量, update_time) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE 布尔变量 = ?, 数值变量 = ?, update_time = ?", CONNECTION_ASYNC);
    PrepareStatement(_AA_REP_Player_Zone_Value, "insert into _数据玩家zone (guid, zone, 布尔变量, 数值变量, update_time) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE 布尔变量 = ?, 数值变量 = ?, update_time = ?", CONNECTION_ASYNC);
    PrepareStatement(_AA_REP_Player_Area_Value, "insert into _数据玩家area (guid, area, 布尔变量, 数值变量, update_time) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE 布尔变量 = ?, 数值变量 = ?, update_time = ?", CONNECTION_ASYNC);
    PrepareStatement(_AA_REP_Player_Instance_Value, "insert into _数据玩家instance (guid, instance, 布尔变量, 数值变量, update_time) VALUES(?,?,?,?,?) ON DUPLICATE KEY UPDATE 布尔变量 = ?, 数值变量 = ?, update_time = ?", CONNECTION_ASYNC);
    
    PrepareStatement(_AA_REP_Map_Map_Value, "insert into _数据地图map (map, 布尔变量, 数值变量, update_time) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE 布尔变量 = ?, 数值变量 = ?, update_time = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(_AA_REP_Item_Zuobiao, "replace into _物品记忆传送 (guid, weizhi, itemEntry, map_name, area_name, map, zone, area, x, y, z, o, 秘境难度, update_time) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);

    PrepareStatement(_AA_REP_Character_Hongbao, "replace into _玩家红包数据 (id, account, guid, 玩家姓名, 发放方式, 红包类型, 红包金额, 红包已领取金额, 红包总数量, 红包已领取数量, 红包描述, 红包领取数量集合, 红包领取玩家集合, update_time) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", CONNECTION_ASYNC);
    PrepareStatement(_AA_DEL_Character_Hongbao, "DELETE FROM _玩家红包数据 WHERE id = ?", CONNECTION_ASYNC);
}
